...
================================================================================
訓練 10 隻美股科技股的 Buy Agent，用於辨識「起漲點」。

階段訓練流程：
- Phase 1 (Pre-training): 整合 10 隻股票的歷史數據進行大規模預訓練
- Phase 2 (Fine-tuning): 針對每隻股票載入預訓練權重，進行個股微調

目標標的: NVDA, MSFT, AAPL, AMZN, META, AVGO, GOOGL, TSLA, NFLX, PLTR
基準指數: ^IXIC (Nasdaq Composite)

Buy Agent 目標: 預測未來 20 個交易日內，最高價報酬率是否達到 +10% 以上
...
FEATURE_COLS = [
    ...
    'Feat_Dist_MA60',
    'Feat_Dist_MA240',
    # 'Feat_Vol_Ratio',  # REMOVED - Volume 相關
    # 新增波動率代理
    'Feat_ATR_Ratio',    # ATR(5) / ATR(20)
    'Feat_HV20',         # 20日年化歷史波動率
    'Feat_Price_Pos',    # 20日價格區間百分比
    'Norm_K', 'Norm_D',
...
# =============================================================================
# 1. 數據載入模組 (地端 CSV 增量更新)
# =============================================================================
def load_or_update_local_csv(ticker: str, start_date: str = "2000-01-01") -> pd.DataFrame:
    """
    載入本地 CSV 資料，若不存在或過期則從 Yahoo Finance 下載並更新
    
    Args:
        ticker: 股票代碼 (e.g., "NVDA", "^IXIC")
        start_date: 資料起始日期
    
    Returns:
        pd.DataFrame: 股價資料 (OHLCV)
    """
    ...
    if os.path.exists(csv_path):
        try:
            ...
            if days_diff <= 1:  # 週末可能 2 天，但我們放寬到 1 天
                need_update = False
                print(f"  ✅ {ticker} (Local, up to {last_date}): {len(df)} rows")
            else:
                print(f"  📥 {ticker} 需要更新 ({last_date} → {today})")
        except Exception as e:
            print(f"  ⚠️ {ticker} CSV 讀取錯誤: {e}")
            df = None
    else:
        print(f"  📥 {ticker} 首次下載...")
    ...
    if need_update:
        try:
            ...
            if len(new_data) > 0:
                # 處理 MultiIndex columns (yfinance 有時會返回)
                ...
                # 儲存到 CSV
                df.to_csv(csv_path)
                print(f"  ✅ {ticker} 更新完成: {len(df)} rows ({df.index[0].date()} ~ {df.index[-1].date()})")
            elif df is None:
                print(f"  ❌ {ticker} 無法下載任何資料")
                return None
        except Exception as e:
            print(f"  ❌ {ticker} 下載錯誤: {e}")
            if df is None:
                return None
...
def fetch_all_stock_data(start_date: str = "2000-01-01") -> dict:
    ...
    if benchmark_df is not None:
        ...
        print(f"  ❌ 無法載入基準指數 {BENCHMARK}，終止程式")
        sys.exit(1)
    ...
    for ticker in TICKERS:
        ...
        if df is not None and len(df) > WARMUP_DAYS:
            ...
            print(f"  ⚠️ {ticker} 資料不足 (需要 > {WARMUP_DAYS} 天)")
    
    print(f"\n[Summary] 成功載入 {len(all_data)} 個標的")
    return all_data
...
def calculate_features(df_in: pd.DataFrame, benchmark_df: pd.DataFrame, 
    ...
    """
    計算特徵 (移除 Volume 相關，新增波動率代理)
    
    Args:
        df_in: 原始 OHLCV 資料
        benchmark_df: 基準指數資料 (^IXIC)
        ticker: 股票代碼
        use_cache: 是否使用快取
...
class BuyEnvHybridV5US(gym.Env):
    """
    Buy Agent 環境 - 美股科技股版本
    
    目標: 預測未來 20 天內最高價報酬率 >= 10%
    獎勵結構 (對稱):
    ...
    def __init__(self, data_dict: dict, is_training: bool = True, balance_tickers: bool = False):
        """
        Args:
            data_dict: {ticker: DataFrame} 包含特徵的資料
            is_training: 是否為訓練模式 (影響抽樣方式)
            balance_tickers: 是否平衡各 Ticker 的抽樣機率 (預訓練時應為 True)
        """
        super().__init__()
        ...
        # 儲存樣本
        self.samples = []           # 所有樣本
        self.pos_samples = []       # 正樣本 (>= 10%)
        self.neg_samples = []       # 負樣本 (< 10%)
        self.ticker_samples = {}    # 依 Ticker 分組的樣本
        ...
        print(f"[BuyEnvV5US] Total: {len(self.samples)} | Pos (≥10%): {len(self.pos_samples)} | Neg: {len(self.neg_samples)}")
    ...
    def reset(self, seed=None, options=None):
        if self.is_training:
            if self.balance_tickers and self.ticker_samples:
                # 預訓練模式：先隨機選 Ticker，再 50/50 類別平衡
    ...
    def step(self, action):
        ...
        is_success = max_ret >= 0.10  # 20 天內最高達 +10%
...
def get_valid_train_ranges(ticker: str, df: pd.DataFrame, train_ranges: list) -> list:
    """
    取得有效的訓練區間 (排除股票尚未上市或暖機期不足的部分)
    
    Args:
        ticker: 股票代碼
        df: 原始資料
        train_ranges: 預設訓練區間列表
    ...
    if len(valid_ranges) < len(train_ranges):
        print(f"  ⚠️ {ticker}: 部分訓練區間無效 (上市日期晚或暖機期不足)")
...
def run_pretraining(train_data: dict, models_path: str):
    ...
    Args:
        train_data: {ticker: DataFrame} 訓練資料
        models_path: 模型儲存路徑
    """
    print("\n" + "=" * 60)
    ...
    # 建立訓練環境 (balance_tickers=True 確保各 Ticker 抽樣均等)
    print("\n🛒 Training Buy Agent (Pre-training on all tickers)...")
...
def run_finetuning(ticker: str, ticker_train_data: dict, ticker_eval_data: dict, 
    ...
    Args:
        ticker: 股票代碼
        ticker_train_data: {ticker: DataFrame} 該股票的訓練資料
        ticker_eval_data: {ticker: DataFrame} 該股票的驗證資料
        models_path: 模型儲存路徑
        manifest: 模型清單字典 (會被更新)
    """
    print(f"\n[Fine-tune] {ticker}")
    print("-" * 40)
    ...
    if not os.path.exists(buy_base_path):
        print(f"  ❌ Base model not found: {buy_base_path}")
        return
    ...
    # Fine-tune 超參數 (LR = pretrain 的 1/10)
    ...
    if os.path.exists(best_path):
        ...
        print(f"  ⚠️ Best model not found, saved last step model")
    ...
    print(f"  ✅ {ticker} Fine-tuning Complete (Train days: {len(train_df)})")
...
def main():
    ...
    print("  目標標的: " + ", ".join(TICKERS))
    print("  基準指數: " + BENCHMARK)
    print("=" * 70)
    ...
    for ticker, df in all_features.items():
        ...
        if len(train_df) > 100:
            ...
            print(f"  ✅ {ticker}: Train {len(train_df)} | Eval {len(eval_df)}")
        else:
            print(f"  ⚠️ {ticker}: 訓練資料不足 ({len(train_df)} rows)")
    
    print(f"\n[Summary] 有效標的: {len(train_data)} / {len(TICKERS)}")
