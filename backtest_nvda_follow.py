...
NVDA 專屬跟單回測腳本
================================================================================
專門針對 NVDA 的回測，支援自訂日期範圍、年度資金注入、信心度分級買入。

功能:
- 回測起始日注入 $2,400，每年第一個交易日再注入 $2,400
- AI 信心度分級買入 (>95%: 25%, 90-95%: 15%, <90%: 不買)
- 市場濾網 (Nasdaq 120MA + DC20 突破)
- 停損停利機制 (-8% / 15%+8%~9%)
- 輸出跟單總結檔案
...
# 信心度門檻與買入比例
CONFIDENCE_HIGH = 0.95      # > 95% → 買入 25%
CONFIDENCE_LOW = 0.90       # 90% - 95% → 買入 15%
...
class NVDAFollowBacktester:
    ...
    def __init__(self, yearly_injection: float = YEARLY_INJECTION):
        ...
        self.capital_pool = 0.0  # 資金池
        self.positions = []      # 持倉列表
        self.trades = []         # 交易紀錄
        self.equity_curve = []   # 淨值曲線
        self.injection_log = []  # 注入紀錄
        self.buy_signals = []    # 買入訊號
        self.sell_signals = []   # 賣出訊號
        self.blocked_signals = []  # 被濾網擋住的訊號
    ...
    def run(self, model: PPO, features_df: pd.DataFrame, 
        ...
        year_first_day_processed = {current_year: True}  # 起始年已處理
        ...
        for i in tqdm(range(len(test_df)), desc=f"  Backtesting {TICKER}", leave=False):
            ...
            for idx, pos in enumerate(self.positions):
                ...
                if sell_reason:
                    ...
                    self.capital_pool += sell_value  # 資金回流
            ...
            if action == 1:  # Agent 發出買入信號
                ...
                if allow_entry:
                    ...
                    if buy_ratio > 0 and self.capital_pool > 0:
                        ...
                        # 跳過無法買到1股的情況 (避免產生無意義的倉位)
                        if invest_amount < price:
                            continue
                        
                        shares = invest_amount / price  # 允許小數股
...
# =============================================================================
# Benchmark: Nasdaq Buy & Hold (同等資金注入)
# =============================================================================
...
def generate_end_date_summary(result: dict, benchmark: dict, output_dir: str, 
    ...
    lines = []
    lines.append("=" * 60)
    lines.append(f"📅 報告日期: {final_date.strftime('%Y-%m-%d')}")
    lines.append("=" * 60)
    
    # 市場數據
    lines.append("[市場數據 - NVDA]")
    lines.append(f"📊 Close: ${final_price:.2f}")
    lines.append("-" * 30)
    lines.append("[市場數據 - Nasdaq]")
    lines.append(f"📊 Close: {final_row.get('Nasdaq_Close', 'N/A'):.2f}")
    lines.append(f"   120MA: {final_row.get('Nasdaq_120MA', 'N/A'):.2f}")
    lines.append(f"   Close > 120MA: {'✅ YES' if final_row.get('Nasdaq_Above_120MA', False) else '❌ NO'}")
    lines.append("-" * 30)
    ...
    lines.append("[帳戶狀態]")
    lines.append(f"   💵 資金池餘額 (Cash):  ${capital_pool:,.2f}")
    ...
    lines.append(f"   📊 總報酬率:           {result['total_return']*100:+.2f}%")
    lines.append("-" * 50)
    ...
    for idx, pos in enumerate(positions, 1):
        ...
        lines.append(f"   #{idx} 買入: {buy_date} @ ${buy_price:.2f} (信心: {conf*100:.1f}%)")
        lines.append(f"       股數: {shares:.4f} | 成本: ${cost:,.2f} | 市值: ${current_value:,.2f}")
    ...
    if action == 1 and allow_entry:
        ...
        if buy_ratio > 0 and capital_pool > 0:
            ...
            lines.append(f"      📊 資金比例: {buy_ratio*100:.0f}% (AI 信心度 {ratio_desc})")
            lines.append(f"      💵 資金池餘額: ${capital_pool:,.2f}")
        else:
            lines.append(f"   📈 【買入建議】: ❌ 不建議買入")
            lines.append(f"      ⚠️ 原因: 信心度不足 ({confidence*100:.1f}% < 90%)")
    else:
        lines.append(f"   📈 【買入建議】: ❌ 不建議買入")
        if action != 1:
    ...
    for idx, pos in enumerate(positions, 1):
        ...
        lines.append(f"      #倉{idx} (市值 ${current_value:,.2f}):")
        lines.append(f"         🛑 硬停損觸發: NVDA 跌至 ${hard_stop_price:.2f} 時賣出")
        ...
        if highest_return >= TRAILING_ACTIVATION:
            ...
            lines.append(f"         📉 移動停利: NVDA 跌至 ${trailing_stop_price:.2f} 時賣出")
            lines.append(f"            → 預計收回: ${trailing_value:,.2f}")
        else:
            lines.append(f"         📉 移動停利: 未啟動 (需漲 {TRAILING_ACTIVATION*100:.0f}%)")
        lines.append("")
    
    lines.append("=" * 60)
    lines.append("📊 績效摘要")
    lines.append("=" * 60)
    lines.append(f"   策略 (NVDA AI Follow):")
    lines.append(f"      總報酬: {result['total_return']*100:+.2f}%")
    ...
    if benchmark:
        lines.append(f"   基準 (Nasdaq B&H):")
        lines.append(f"      總報酬: {benchmark['total_return']*100:+.2f}%")
...
def plot_equity_curve(result: dict, benchmark: dict, output_dir: str,
    ...
    ax.set_title(f'NVDA AI Follow 策略淨值曲線\n{start_date} ~ {end_date}', fontsize=14)
    ax.set_xlabel('Date')
...
# =============================================================================
def parse_args():
    parser = argparse.ArgumentParser(description='NVDA 專屬跟單回測腳本')
    parser.add_argument('--start', type=str, default=DEFAULT_START,
                        help=f'回測起始日期 (預設: {DEFAULT_START})')
    parser.add_argument('--end', type=str, default=DEFAULT_END,
                        help=f'回測結束日期 (預設: {DEFAULT_END})')
...
def main():
    ...
    # 日期驗證：如果起始日晚於結束日，將結束日設為今天
    if pd.Timestamp(start_date) > pd.Timestamp(end_date):
        end_date = datetime.now().strftime('%Y-%m-%d')
        print(f"⚠️  起始日晚於預設結束日，自動將結束日設為今天: {end_date}")
    
    # 動態輸出目錄 (放在專屬目錄內，按日期區分)
    start_short = start_date.replace('-', '')
    end_short = end_date.replace('-', '')
    ...
    print(f"  資金注入: ${YEARLY_INJECTION:,}/年")
    print(f"  信心度門檻: >{CONFIDENCE_HIGH*100:.0f}%→{BUY_RATIO_HIGH*100:.0f}%, "
          f"{CONFIDENCE_LOW*100:.0f}%-{CONFIDENCE_HIGH*100:.0f}%→{BUY_RATIO_LOW*100:.0f}%")
    ...
    if model is None:
        print(f"❌ 無法載入 {TICKER} 模型")
        return
    print(f"✅ {TICKER} 模型載入成功")
    ...
    print("📊 回測績效")
    print("=" * 70)
    print(f"   {'指標':<20} | {'NVDA AI Follow':<20} | {'Nasdaq B&H':<20}")
    print("-" * 70)
    print(f"   {'總報酬':<20} | {result['total_return']:>18.1%} | {benchmark['total_return'] if benchmark else 'N/A':>18.1%}")
    ...
    print(f"   {'交易次數':<20} | {result['total_trades']:>18} | {'N/A':>18}")
    print(f"   {'勝率':<20} | {result['win_rate']:>18.1%} | {'N/A':>18}")
    print("=" * 70)
