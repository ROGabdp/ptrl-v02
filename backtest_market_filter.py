...
"""
================================================================================
US Tech Stock - 深度回測 (市場濾網版本)
================================================================================
引入「120 日均線」與「20 日唐其安通道」作為市場濾網，優化 MDD 與 Sharpe。

市場濾網邏輯:
- 條件 1 (多頭市場): Nasdaq > 120MA → 准許買入
- 條件 2 (逆勢突破): Nasdaq <= 120MA 且 個股 > DC20_High → 准許買入
- 其餘情況: 即使 Agent 發出信號也保持空手

回測設定:
- 期間: 2017-10-16 ~ 2023-10-15
- 信心度門檻: > 90%
- 停損停利: -8% / 8% / 9% (敏感度分析建議)
...
def prepare_market_filter(benchmark_df: pd.DataFrame, ticker_df: pd.DataFrame) -> pd.DataFrame:
    ...
    Args:
        benchmark_df: Nasdaq (^IXIC) 資料
        ticker_df: 個股資料
    
    Returns:
        pd.DataFrame: 包含濾網指標的合併資料
    """
    # Nasdaq 120MA
    ...
    ticker_df['DC20_High'] = ticker_df['High'].rolling(20).max().shift(1)  # 前一天的 20 日最高
...
class FilteredBacktester:
    ...
    def __init__(self, initial_capital: float = INITIAL_CAPITAL, use_filter: bool = True):
        ...
        self.sell_signals = []
        self.blocked_signals = []  # Agent 發出信號但被濾網擋住
...
def plot_trade_signals_filtered(ticker: str, features_df: pd.DataFrame, 
    ...
    ax1.set_title(f'{ticker} - 交易訊號 (120MA + DC20 濾網)', fontsize=13)
...
def print_comparison_summary(filtered: list, unfiltered: list, benchmark: dict):
    ...
    print(f"   濾網: Nasdaq 120MA + 個股 DC20 突破")
    print("=" * 110)
    ...
    for f, u in zip(filtered, unfiltered):
        ...
        if u:
            ...
            print(f"{'':>8} | {'MDD Δ':>12} | {'':>10} | {'':>8} | {'':>8} | {mdd_diff:>+9.1%} | {'':>6} | {'':>6}")
...
def parse_args():
    ...
    parser.add_argument('--tickers', type=str, nargs='+', default=TARGET_TICKERS,
                        help=f'回測標的 (預設: {TARGET_TICKERS})')
    parser.add_argument('--no-filter', action='store_true',
                        help='停用市場濾網 (僅執行無濾網版本)')
...
def main():
    ...
    for ticker in tickers:
        ...
        if result_filtered:
            ...
            print(f"  📊 Filtered: Return={result_filtered['total_return']:.1%} | MDD={result_filtered['max_drawdown']:.1%} | MDD Δ={mdd_diff:+.1%}")
